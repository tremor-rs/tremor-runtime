...
mod tests {
    ...
    pub fn decode() {
        ...
        let decoded = codec.decode(EventValue::Raw(s)).unwrap();
    ...
    pub fn encode() {
        ...
        let encoded = codec.encode(EventValue::JSON(s)).unwrap();
        ...
        match &encoded {
            EventValue::Raw(r) => assert_eq!(*r, influx.try_to_bytes().unwrap()),
            _ => unreachable!(),
        }
    }
    ...
    pub fn encode_mixed_bag() {
        ...
        let s = json!({
            ...
            "tags": tags,
            "fields": hashmap!{"temp=erature" => num_f(82.0), r#"too\ \\\"hot""# => Value::Bool(true)},
        ...
        match &encoded {
            EventValue::Raw(r) => {
                ...
                assert_eq!(str::from_utf8(r).unwrap(), raw);
            }
    ...
    pub fn round_trip_all_cases() {
        ...
        pairs.iter().for_each(|case| {
            ...
            let encoded = codec.encode(EventValue::JSON(case.1.clone())).unwrap();

            match &encoded {
                EventValue::Raw(r) => {
                    let decoded = codec.decode(EventValue::Raw(r.clone())).unwrap();
