...
async fn handle_error_with_fail(
    ...
    reply_tx
        .send(sink::Reply::Response(
            ERR,
            Tnt::create_error_response(ids, e, event_origin_uri),
        ))
        .await?;
    Ok(())
}
...
impl Tnt {
    ...
    fn create_error_response(event_id: &EventId, e: &str, origin_uri: &EventOriginUri) -> Event {
        let mut error_data = tremor_value::Object::with_capacity(2);
        ...
        let mut meta = tremor_value::Object::with_capacity(1);
...
mod test {
    ...
    fn message_to_event_ok() -> Result<()> {
        let sink_url = TremorURL::parse("/offramp/tnt/instance")?;
    ...
    async fn test_failed_connection_lifecycle() -> Result<()> {
        ...
        let url = TremorURL::parse("/offramp/tnt/instance")?;
