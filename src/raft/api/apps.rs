...
async fn uninstall_app(
    extract::State(state): extract::State<APIRequest>,
    extract::OriginalUri(uri): extract::OriginalUri,
    extract::Path(app_id): extract::Path<alias::App>,
    ...
    if let Some(app) = app {
        if !app.instances.is_empty() {
            return Err(AppError::HasInstances(
                ...
                app.instances
                    .keys()
                    .map(|alias| alias::Flow::new(app_id.clone(), alias))
                    .collect(),
            )
            .into());
        }
...
async fn list(State(state): State<APIRequest>) -> APIResult<Json<HashMap<alias::App, AppState>>> {
...
async fn start(
    extract::State(state): extract::State<APIRequest>,
    extract::OriginalUri(uri): extract::OriginalUri,
    extract::Path((app_id, flow_id)): extract::Path<(alias::App, alias::FlowDefinition)>,
    Json(body): Json<TremorStart>,
) -> APIResult<Json<alias::Flow>> {
    let instance_id = body.instance.clone();
    let flow_id = alias::Flow::new(app_id, flow_id);

    let app = timeout(
        API_WORKER_TIMEOUT,
        state.raft_manager.get_app_local(flow_id.app_id().clone()),
    )
    .await??;
    ...
    if let Some(app) = app {
        ...
        if let Some(flow) = app.app.flows.get(flow_id.instance_id()) {
            ...
            return Err(AppError::FlowNotFound(flow_id).into());
        }
    } else {
        return Err(AppError::AppNotFound(flow_id.app_id().clone()).into());
    }
    let request = TremorRequest::Apps(AppsCmd::Deploy {
        app: flow_id.app_id().clone(),
        flow: flow_id.instance_id().into(),
...
async fn manage_instance(
    extract::State(state): extract::State<APIRequest>,
    extract::OriginalUri(uri): extract::OriginalUri,
    extract::Path((app_id, flow_id)): extract::Path<(alias::App, String)>,
    Json(body): Json<TremorInstanceState>,
) -> APIResult<Json<alias::Flow>> {
    // TODO: this is not only for this but all the API functions as we're running in a potentially
    ...
    // handled
    let instance_id = alias::Flow::new(app_id.clone(), flow_id);
...
async fn stop_instance(
    ...
    debug!("Stop instance request for {app_id}/{flow_id}");
    let instance_id = alias::Flow::new(app_id.clone(), flow_id);
