...
impl ClientContext for LoggingConsumerContext {
    fn error(&self, error: KafkaError, reason: &str) {
        ...
        if let e @ KafkaError::Subscription(_)
            | e @ KafkaError::ClientConfig(_, _, _, _)
            | e @ KafkaError::ClientCreation(_)
            // TODO: what else?
            | e @ KafkaError::Global(RDKafkaErrorCode::UnknownTopicOrPartition)
            | e @ KafkaError::Global(RDKafkaErrorCode::UnknownTopic) = error {
        ...
        } = &mut self
        {
            ...
            match r {
                ...
                Some(Err(e)) => match e {
                    ...
                    | KafkaError::MessageConsumption(
                        e @ RDKafkaErrorCode::TopicAuthorizationFailed,
                    )
                    | KafkaError::MessageConsumption(e @ RDKafkaErrorCode::UnknownTopic) => {
                        error!(
