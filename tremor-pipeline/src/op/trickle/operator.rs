...
impl TrickleOperator {
    pub fn with_stmt(id: String, stmt_rentwrapped: tremor_script::StmtRentalWrapper) -> TrickleOperator {
        use crate::op;
        ...
        let op: Box<dyn Operator> = match stmt {
            tremor_script::ast::Stmt::OperatorDecl(ref op) => {
                ...
                match (op.kind.module.as_str(), op.kind.operation.as_str()) {
                    ("debug", "history") => { 
                        Box::new(op::debug::history::EventHistory {
                            config: op::debug::history::Config {
                                ...
                                op: op.params.as_ref().expect("expected op to be configured")["op"].to_string(),
                                name: op.params.as_ref().expect("expected name to be configured")["name"].to_string(),
                            },
                            id: op.id.to_string(),
                        })
                    }
                    ("generic", "backpressure") => { 
                        Box::new(op::generic::backpressure::Backpressure {
                            config: op::generic::backpressure::Config {
                                // FIXME hygienic error handling
                                timeout: op.params.as_ref().expect("expected timeout to be configured")["timeout"].cast_f64().expect("snot badger"),
                                steps: op.params.as_ref().expect("expected steps to be configured")["steps"].as_array().expect("expected to be an array type").iter().map(|x| x.as_u64().expect("snot biscuit")).collect(),
                            },
                            backoff: 0,
                            last_pass: 0,
                        })
                    ...
                    ("generic", "batch") => { 
                        // FIXME hygienic error handling
                        let count = op.params.as_ref().expect("expected timeout to be configured")["count"].as_u64().expect("snot badger") as usize;
