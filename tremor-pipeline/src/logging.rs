...
use tremor_script::{registry::{Registry, FResult, mfa, FunctionError, Mfa}, TremorFn, EventContext, Value, TremorFnWrapper};
...
impl PluggableLoggingFm {
	...
	fn debug() -> Self{
		Self { name: "debug".into() }
	}

	fn error() -> Self{
		Self { name: "error".into() }
	}
...
impl TremorFn for PluggableLoggingFm {
    ...
    fn invoke<'event, 'c>(
        ...
        if args.is_empty() {
            return Err(FunctionError::BadArity{mfa: this_mfa(), calling_a: args.len()});
        }
            ...
            .and_then(|(f, arg_stack)| Some((f.as_str()?, arg_stack)))
        {
			...
			let out = if arg_stack.len() > 1 {
				...
				if let Some(_) = arg_stack.as_object() { // retrieve format args as a hashmap
					...
					positional_formatting(this_mfa(), format, &Value::Array(vec![arg_stack.clone()]))
				}
			} else { // arg_stack.len() < 1 (== 0)
				positional_formatting(this_mfa(), format, &Value::Array(vec![])) // `arg_stack` is empty
			};


			match out {
				Ok(out_ok) => {
					let mut result = tremor_value::value::Object::default();
					...
					match self.name.as_str() {
						...
						"warn"  => {
						  ...
						  func  => Err(FunctionError::RuntimeError{mfa: this_mfa(), error: format!("Unknown logging function \"{func}\"")})
					}
				},
				err => err
			}
...
fn positional_formatting<'event>(this_mfa: Mfa, format: &str, arg_stack: &Value) -> FResult<Value<'event>> {
...
fn named_formatting<'event>(this_mfa: Mfa, format: &str, arg_stack: &Value) -> FResult<Value<'event>> {
...
fn format_tremor_value(value: &Value) -> Result<String> {
	...
	if let Some(bytes) = value.as_bytes() {
		result.push_str(bytes.to_vec().iter().map(|int: &u8| *int as char).collect::<String>().as_str());
		return Ok(result);
	}
...
#[cfg(test)]
mod test {
use tremor_script::{ Registry, Object, literal};
...
use super::*;
    ...
    fn test_info() {
        ...
        let res = fun.invoke(&ctx,&[&Value::from("info {key2} {key}"),&Value::from(obj)]);
		assert_eq!(Ok(literal!({"args": "info [value2, p, ojn] value", "level": "INFO"})), res);
    }
    ...
    fn test_warn() {
        ...
        let res = fun.invoke(&ctx,&[&Value::from("info {key2} {key}"),&Value::from(obj)]);
		assert_eq!(Ok(literal!({"args": "info [value2, p, ojn] value", "level": "WARN"})), res);
    }
    ...
    fn test_debug() {
        ...
        let res = fun.invoke(&ctx,&[&Value::from("info {key2} {key}"),&Value::from(obj)]);
        assert_eq!(Ok(literal!({"args": "info [value2, p, ojn] value", "level": "DEBUG"})), res);
    }
    ...
    fn test_error() {
        ...
        let res = fun.invoke(&ctx,&[&Value::from("info {key2} {key}"),&Value::from(obj)]);
        assert_eq!(Ok(literal!({"args": "info [value2, p, ojn] value", "level": "ERROR"})), res);
    }
    ...
    fn test_trace() {
        ...
        let res = fun.invoke(&ctx,&[&Value::from("info {key2} {key}"),&Value::from(obj)]);
        assert_eq!(Ok(literal!({"args": "info [value2, p, ojn] value", "level": "TRACE"})), res);
    }
