...
use tremor_runtime::temp_network::ws::{UrMsg, WsMessage};

pub async fn get(req: Request) -> Result<Response> {
    let uring = &req.state().world.uring;

    let key: String = req.param("id").unwrap().to_string();
    let id = key.clone().into_bytes();
    //info!("GET /kv/{}", key);

    let (tx, mut rx) = bounded(64);
    uring.try_send(UrMsg::KVGet(id, tx)).unwrap();
...
pub async fn post(mut req: Request) -> Result<Response> {
    let key: String = req.param("id").unwrap().to_string();
    let id = key.clone().into_bytes();
    let body: PostBody = req.body_json().await?;

    let uring = &req.state().world.uring;

    let (tx, mut rx) = bounded(64);
    uring
        .try_send(UrMsg::KVPut(id, body.value.into_bytes(), tx))
        .unwrap();
