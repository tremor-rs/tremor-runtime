...
async fn no_connection() -> anyhow::Result<()> {
    ...
    let harness = Harness::new("google pub sub", &Builder::default(), &connector_yaml).await?;
...
async fn no_hostname() -> anyhow::Result<()> {
    ...
    let harness = Harness::new("google pubsub", &Builder::default(), &connector_yaml).await;
...
async fn simple_publish() -> anyhow::Result<()> {
    ...
    let mut harness = Harness::new("google pubsub", &Builder::default(), &connector_yaml).await?;
    harness.start().await?;
