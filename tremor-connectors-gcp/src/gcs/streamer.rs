...
use super::{resumable_upload_client::ResumableUploadClient, Error};
...
impl ConnectorBuilder for Builder {
    fn connector_type(&self) -> ConnectorType {
        CONNECTOR_TYPE.to_string().into()
    }

    async fn build_cfg(
        &self,
        alias: &alias::Connector,
        _config: &config::Connector,
...
impl<Client: ResumableUploadClient> Common for GCSObjectStorageSinkImpl<Client> {
...
#[async_trait::async_trait]
impl<Client: ResumableUploadClient + Send + Sync> SinkImpl<GCSUpload>
...
pub(crate) mod tests {
    ...
    pub(crate) struct TestUploadClient {
        counter: AtomicUsize,
        running: BTreeMap<url::Url, (ObjectId, Vec<BufferPart>)>,
        finished: BTreeMap<url::Url, (ObjectId, Vec<BufferPart>)>,
        deleted: BTreeMap<url::Url, (ObjectId, Vec<BufferPart>)>,
        inject_failure: bool,
    }
    ...
    impl ResumableUploadClient for TestUploadClient {
        async fn start_upload(
            ...
            if self.inject_failure {
                error!("[TestUploadClient] Injected failure in start_upload: {url}");
                return Err(Error::Upload(StatusCode::INTERNAL_SERVER_ERROR).into());
            }
            debug!("[TestUploadClient] Starting upload for {file_id} / {url}");
        ...
        async fn upload_data(&mut self, url: &url::Url, part: BufferPart) -> anyhow::Result<usize> {
            if self.inject_failure {
                error!("[TestUploadClient] Injected failure in upload_data: {url}");
                return Err(Error::Upload(StatusCode::INTERNAL_SERVER_ERROR).into());
            }
            if let Some((file_id, buffers)) = self.running.get_mut(url) {
                debug!("[TestUploadClient] Upload data for {file_id} / {url}");
        ...
        async fn finish_upload(&mut self, url: &url::Url, part: BufferPart) -> anyhow::Result<()> {
            if self.inject_failure {
                error!("[TestUploadClient] Injected failure in finish_upload: {url}");
                return Err(Error::Upload(StatusCode::INTERNAL_SERVER_ERROR).into());
            }
            if let Some((file_id, mut buffers)) = self.running.remove(url) {
                debug!("[TestUploadClient] Finish upload for {file_id} / {url}",);
                buffers.push(part);
        ...
        async fn delete_upload(&mut self, url: &url::Url) -> anyhow::Result<()> {
            ...
            if let Some(upload) = self.running.remove(url) {
                debug!("[TestUploadClient] Finish upload for {} {url}", upload.0);
        ...
        async fn bucket_exists(
            ...
            if self.inject_failure {
                error!("[TestUploadClient] Injected failure in bucket_exists: {url}");
                return Err(
                    Error::Bucket(bucket.to_string(), StatusCode::INTERNAL_SERVER_ERROR).into(),
                );
            }
            debug!("[TestUploadClient] Bucket exists: {}", bucket);
            Ok(true)
        }
    }
    ...
    async fn yolo_happy_path() -> anyhow::Result<()> {
        let upload_client_factory = Box::new(|_config: &Config| {
            info!("Creating upload client for test");
            Ok(TestUploadClient::default())
        });
        ...
        let (connection_lost_tx, _) = channel(10);
        ...
        let event = Event {
            ...
            data: (
                ...
                literal!({
                    // we start a 3 event upload to happy.txt
                    "gcs_streamer": {
                        "name": "happy.txt",
                        "bucket": "yolo"
                    }
        ...
        assert_eq!(b"badg\",\"er\"", buffer.data());

        let buffer = buffers.pop().expect("no data");
        assert_eq!(0, buffer.start());
        assert_eq!(10, buffer.len());
        assert_eq!(b"{\"snot\":[\"", buffer.data());
        ...
        let event = Event {
            ...
            data: (
                ...
                {
                    // first event for sad.txt, this should close out the happy.txt upload
                    "data": {
        ...
        // two files should be finished, happy.txt and sad.txt
        ...
        assert_eq!(10, buffer.start());
        assert_eq!(10, buffer.len());
        assert_eq!(b"e,false,nu", buffer.data());

        let buffer = buffers.pop().expect("no data");
        assert_eq!(0, buffer.start());
        assert_eq!(10, buffer.len());
        assert_eq!(b"[1,2,3,tru", buffer.data());
        ...
        assert_eq!(b"adger\"", buffer.data());

        let buffer = buffers.pop().expect("no data");
        assert_eq!(20, buffer.start());
        assert_eq!(b"]}\"snot\"\"b", buffer.data());

        assert!(upload_client(&mut sink).deleted_uploads().is_empty());

        sink.finalize(&context, &mut serializer).await?;
        ...
        let (file_id, mut buffers) = finished.pop().expect("no data");
        assert_eq!(file_id, ObjectId::new("yolo", "sad.txt"));
        assert_eq!(buffers.len(), 3);
        let last = buffers.pop().expect("no data");
        assert_eq!(last.start(), 20);
        assert_eq!(last.data(), b"ll]");
    ...
    pub async fn consistent_happy_path() -> anyhow::Result<()> {
        ...
        // Send a first event to woah/test.txt
        ...
        assert_eq!(1, uploads.len());
        let (file_id, buffers) = uploads.pop().expect("no data");
        assert_eq!(ObjectId::new("bucket1", "file1.txt"), file_id);
        assert!(buffers.is_empty());
        ...
        assert_eq!(b"qrstuvwxyz", &part.data());
        let part = buffers.pop().expect("no data");
        assert_eq!(10, part.start());
        assert_eq!(10, part.len());
        assert_eq!(b"ghijklmnop", &part.data());
        let part = buffers.pop().expect("no data");
        assert_eq!(0, part.start());
        assert_eq!(10, part.len());
        assert_eq!(b"{}[\"abcdef", &part.data());
        ...
        assert_eq!(1, uploads.len());
        let (file_id, buffers) = uploads.pop().expect("no data");
        assert_eq!(ObjectId::new("bucket2", "file1.txt"), file_id);
        assert!(buffers.is_empty());
        ...
        assert_eq!(1, uploads.len());
        let (file_id, buffers) = uploads.pop().expect("no data");
        assert_eq!(ObjectId::new("bucket2", "file2.txt"), file_id);
        assert!(buffers.is_empty());
        ...
        // what is happening - THIS IS WHERE WE FAIL
        assert_eq!(file_id, ObjectId::new("bucket2", "file1.txt"));
        assert_eq!(buffers.len(), 1);
        let last = buffers.pop().expect("no data");
        assert_eq!(last.start(), 0);
        assert_eq!(last.len(), 2);
        assert_eq!(&last.data(), b"42");
    ...
    async fn connector_yolo_mode() -> anyhow::Result<()> {
        ...
        let addr = spawn(&alias, &mut connector_id_gen, &builder, cfg, &kill_switch).await?;
