...
# followers
cargo run -p tremor-cli -- --instance 2 server run --logger-config .backups/logger.yaml --cluster-host "127.0.0.1:8081" --cluster-peer "127.0.0.1:8080" --cluster-peer "127.0.0.1:8082" --api-host "127.0.0.1:9899"
cargo run -p tremor-cli -- --instance 3 server run --logger-config .backups/logger.yaml --cluster-host "127.0.0.1:8082" --cluster-peer "127.0.0.1:8081" --cluster-peer "127.0.0.1:8082" --api-host "127.0.0.1:9897"
...
curl -s http://localhost:9899/status | jq
...
curl http://localhost:9899/kv/snot
