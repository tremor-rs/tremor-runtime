...
cargo run -p tremor-cli -- --instance 1 server run --logger-config .backups/logger.yaml \
  --cluster-host "127.0.0.1:8080" \
  --cluster-peer "ws://127.0.0.1:8081" \
  --temp-cluster-host "127.0.0.1:7070" \
  --network-host "127.0.0.1:9090" \
  --cluster-bootstrap
...
cargo run -p tremor-cli -- --instance 2 server run --logger-config .backups/logger.yaml \
  --cluster-host "127.0.0.1:8081" \
  --cluster-peer "ws://127.0.0.1:8080" \
  --temp-cluster-host "127.0.0.1:7071" \
  --network-host "127.0.0.1:9091" \
  --api-host "127.0.0.1:9897"
#
