...
#![cfg(feature = "elasticsearch-integration")]
...
async fn connector_elastic() -> anyhow::Result<()> {
    ...
    let mut harness = Harness::new("test", &elastic::Builder::default(), &connector_config).await?;
...
async fn elastic_https_invalid_url() -> anyhow::Result<()> {
    ...
    assert!(
        Harness::new("test", &elastic::Builder::default(), &connector_config,)
            .await
            .is_err()
    );
    Ok(())
...
async fn send_one_event(harness: &mut Harness) -> anyhow::Result<()> {
