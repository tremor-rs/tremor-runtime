...
use crate::{
    create_bucket, get_client, random_bucket_name, spawn_docker, wait_for_s3, EnvHelper,
    MINIO_REGION, MINIO_ROOT_PASSWORD, MINIO_ROOT_USER,
};
...
use tremor_connectors::{harness::Harness, impls::s3};
use tremor_system::{
...
async fn no_connection() -> anyhow::Result<()> {
    ...
    let harness = Harness::new("test", &s3::streamer::Builder::default(), &connector_yaml).await?;
...
async fn connector_s3_consistent() -> anyhow::Result<()> {
    ...
    let mut harness =
        Harness::new("test", &s3::streamer::Builder::default(), &connector_yaml).await?;
    harness.start().await?;
