...
use tremor_common::{alias, asy::file};
use tremor_runtime::{
    raft::{
    ...
    pub(crate) async fn run(self, api: &str) -> Result<()> {
        ...
        match self {
            ...
            AppsCommands::Start {
                ...
                let flow = flow.map_or_else(
                    || alias::FlowDefinition::from("main"),
                    alias::FlowDefinition,
                );
                let app_id = alias::App::new(app);
                let instance_id = alias::Flow::new(app_id, instance);
                match client
            ...
            AppsCommands::Stop { app, instance } => {
                let instance_id = alias::Flow::new(app, instance);
